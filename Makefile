...
articles/%.html: articles/%.md
	$(tool_html_of_md) `cat $(<:%.md=%.meta)` css_href=../../css/style.css div_class=\"article numbering text_margins\" input=\"$<\" > $@

# Rule for generating html from any post
posts/%.html: posts/%.md
	$(tool_html_of_md) `cat $(<:%.md=%.meta)` css_href=../../css/style.css div_class=\"article text_margins\" input=\"$<\" > $@
...
topic_%.html: topic_%.md
	$(tool_html_of_md) title=\"Topic: $*\" topics=\"$*\" div_class=\"article text_margins index_lists\" input=\"$<\" > $@
...
all_Articles.html: all_Articles.md
	$(tool_html_of_md) title=Articles div_class=\"article text_margins index_lists\" input=\"$<\" > $@
...
all_Posts.html: all_Posts.md
	$(tool_html_of_md) title=Posts div_class=\"article text_margins index_lists\" input=\"$<\" > $@
...
index.html: index.md
	$(tool_html_of_md) div_class=\"article text_margins index_lists\" input=\"$<\" > $@
